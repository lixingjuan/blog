...
简介
1. 原型链继承：思想：将子类型实例的`prototype`指向超类型实例
1. 借用构造函数：
   1. 思想：通过 `call()` 或 `apply()`方法，在（将来）新创建的对象上执行超类型构造函数；
   2. 优点：
      1. 可向超类型传值
      2. 可为子类型实例单独定义属性
   3. 缺点：函数内部定义，代码无法复用；
2. 组合式继承：结合原型链继承和借用构造函数继承，
   1. 解决原型链继承：实例引用类型属性共享 和 无法向超类型传值
   2. 解决借用构造函数继承：代码不可复用问题；
3. 原型式继承：可以在不必预先定义构造函数的情况下实现继承，本质是执行给定对象的浅拷贝。而复制后的副本还可以得到进一步的改造；
4. 寄生式继承: 为了解决组合继承模式由于多次调用超类型构造函数而导致的低效率问题
5. 组合寄生式继承: 集寄生式继承和组合继承的优点于一身，是实现基于类型积尘的最优方式
...
**实现思路**：使用原型链实现对 **原型属性和方法** 的继承，而通过借用构造函数实现对 **实例属性的继承**
...
组合继承
优点
1. 避免了原型链和借用构造函数的缺陷，融合了他们的优点，是最常用的继承方式；
2. 同时，instanceof 和isPrototypeOf() 也能够用于识别基于组合继承创建的对象； 
...
- 思路：创建一个仅用于封装继承过程的函数，在函数内部以某种方式增强对象，最后返回对象
...
> 基本思路：不必为了指定子类型的原型而调用超类型的构造函数，我们所需要的无非就是超类型原型的一个副本，本质上，就是使用寄生式继承来继承超类型的原型，然后再将结果指定给子类型的原型；
