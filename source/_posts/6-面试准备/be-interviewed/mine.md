...
2. 怎么实现资源同步下载 -- defer
3. dispaly 实现超出换行？
4. useState 和 class 的 setState 的区别？ 如果同时修改一个对象
5. 如果想让prop 和state 改变，但是不让组件渲染，怎么做？--- useRef
6. 🔴性能优化！！！
   1. 代码(react)
   2. webpack打包
   3. 网络
7. 🔴webpack 拆包
8. docker 有没有了解
9. 一个函数，有两种输入，（复杂类型），ts怎么处理？———— key in 操作符
10. 使用JSON.stringify做深拷贝有什么问题？ ———— 复杂类型丢失
11. 箭头函数特性 （在问this）
    1. 箭头函数能不能做构造函数？
